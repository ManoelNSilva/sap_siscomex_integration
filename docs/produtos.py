...
from app.config import PREFIX_CATP, LINK_PROD, ROOT_CPF_CNPJ, URL_INSERT_PRODUCT, INSERT_PRODUCT_FILE, DISABLE_PRODUCT_FILE
...
def consulta_produtos():
    ...
    response = requests.get( url, params={"cpfCnpjRaiz": ROOT_CPF_CNPJ},
...
def desativa_produtos_lote():
    ...
    for codigo in codigos:
        ...
        item = {"codigo": codigo, "status_code": response.status_code, "sucesso": response.ok}
        if response.ok:
            sucesso += 1
